...
// Witch and Tankifier
bool bTankifier, bl4d_tank_damage_announce;
...
void Findl4d_tank_damage_announce()
{
	bl4d_tank_damage_announce = LibraryExists("l4d_tank_damage_announce");
}
...
void UpdateSpecHudBasedOnConVar()
{
...
public void OnRoundIsLive()
{
    ...
    // Enable spechud for all spectators if the ConVar is set
    UpdateSpecHudBasedOnConVar();
}
...
public Action ToggleSpecHudCmd(int client, int args)
{
	...
	bSpecHudPersistent[client] = bSpecHudActive[client]; // Update persistent HUD state
...
public Action HudDrawTimer(Handle hTimer)
{
	...
	for (int client = 1; client <= MaxClients; ++client)
	{
		...
		switch (team)
		{
			case 3:
			{
				if (tankHud != null && bTankHudActive[client])
				{
			...
			case 1:
			{
				...
				else if (specHud != null && bSpecHudActive[client])
				{
					specHud.Send(client, DummySpecHudHandler, 3);
...
bool FillTankInfo(Panel hSpecHud, bool bTankHUD = false)
{
	...
	static char info[64], sLine[64];
	static char name[MAX_NAME_LENGTH];

	if (bTankHUD)
	{
		FormatEx(sLine, sizeof(sLine), "%s : Tank HUD", sReadyCfgName);
		ValvePanel_ShiftInvalidString(sLine, sizeof(sLine));
		DrawPanelText(hSpecHud, sLine);

		int len = strlen(sLine);
		for (int i = 0; i < len; ++i) sLine[i] = '_';
		DrawPanelText(hSpecHud, sLine);
		...
		if(bl4d_tank_damage_announce)
		{
			...
			TotalDamage=TFA_TotalDmg(tank);

			FormatEx(info, sizeof(info), "Punches: %d | Rocks: %d | Hittables: %d", Punch, Rock, Hittable);
			DrawPanelText(hSpecHud, info); // "Punch: %d | Rock: %d | Hittable: %d

			FormatEx(info, sizeof(info), "           Total Damage: %d", TotalDamage);
			DrawPanelText(hSpecHud, info); //   TotalDamage: %d

			DrawPanelText(hSpecHud, sLine); //_____________________________________
		}
	}
	else
	{
	...
	if(bl4d_tank_damage_announce)
	{
		...
		{
			FormatEx(info, sizeof(info), "Frustr.  : AI / %s%d:%s%d", (UpTime/60 < 10) ? "0" : "", (UpTime/60), (UpTime%60 < 10) ? "0" : "", UpTime%60);
		}
		DrawPanelText(hSpecHud, info);
	}
	else
	{
		...
		{
			FormatEx(info, sizeof(info), "Frustr.  : AI");
		}
		DrawPanelText(hSpecHud, info);
	}
