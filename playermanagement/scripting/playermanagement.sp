...
{
	name		= "Player/Team Management Plugin",
	author		= "CanadaRox, heavy, heize",
...
ConVar  disable_spectate_on_round_end;
...
{
	AutoExecConfig_SetFile("player_team_management");
	...
	g_cvarBlockInTank		  = CreateConVar("sm_blockspecintank", "0", "Block survivors from switching to spect while in tank", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	disable_spectate_on_round_end = AutoExecConfig_CreateConVar("disable_spectate_on_round_end", "0", "Mode for disabling spectate on round end: 0 = Keep enabled, 1 = Disable", FCVAR_NONE, true, 0.0, true, 1.0);
...
{
	RequestFrame(OnFrame_RoundStart);
}

void OnFrame_RoundEnd()
{
	if (disable_spectate_on_round_end.BoolValue)
	{
		sm_allow_spectate_command.SetBool(false);
	}
}

void OnFrame_RoundStart()
{
	sm_allow_spectate_command.SetBool(true);
}
...
Action Spectate_Cmd(int client, int args)
{
	...
	{
		CPrintToChat(client, "%t %t", "Tag", "SpectateDisabled");
		return Plugin_Handled;
	}
...
Action FSpec_Cmd(int client, int args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "%t %t: fspec <player1> <player2> ... <playerN>", "Tag", "Usage");
		return Plugin_Handled;
	}
	...
	for (int i = 1; i <= args; i++)
	{
		...
		for (int j = 0; j < targetCount; j++)
		{
			...
			if (IsClientInGame(target))
			{
				pendingSwaps[target] = L4D2Team_Spectator;
			}
		}
	}
...
Action FSur_Cmd(int client, int args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "%t %t: fsur <player1> <player2> ... <playerN>", "Tag", "Usage");
		return Plugin_Handled;
	}
	...
	for (int i = 1; i <= args; i++)
	{
		...
		for (int j = 0; j < targetCount; j++)
		{
			...
			if (IsClientInGame(target))
			{
				pendingSwaps[target] = L4D2Team_Survivor;
			}
		}
	}
...
Action FInf_Cmd(int client, int args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "%t %t: finf <player1> <player2> ... <playerN>", "Tag", "Usage");
		return Plugin_Handled;
	}
	...
	for (int i = 1; i <= args; i++)
	{
		...
		for (int j = 0; j < targetCount; j++)
		{
			...
			if (IsClientInGame(target))
			{
				pendingSwaps[target] = L4D2Team_Infected;
			}
		}
	}
...
stock void ApplySwaps(int sender, bool force)
{
	...
	/* Now let's try to put them on teams */
	for (int client = 1; client <= MaxClients; client++)
	{
		if (IsClientInGame(client) && pendingSwaps[client] != L4D2Team_None)
		{
			...
			{
				CPrintToChat(client, "%t %t", "Tag", "MovedToTeam", pendingSwaps[client]);
				CPrintToChatAll("%t %t %t", "Tag", "PlayerMovedToTeam", client, pendingSwaps[client]);
			}
