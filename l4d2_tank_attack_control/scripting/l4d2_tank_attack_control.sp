...
public Plugin myinfo = 
{
	...
	author = "vintik, CanadaRox, Jacob, Visor, heize",
	description = "",
	version = "0.7.4",
...
void Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	delete g_hTimer[tank];
	g_hTimer[tank] = CreateTimer(1.0, Timer_Event_PlayerSpawn, tank);
}

Action Timer_Event_PlayerSpawn(Handle timer, int tank)
{
	g_hTimer[tank] = null;

	if (!tank || !IsClientInGame(tank) || GetClientTeam(tank) != 3 || IsFakeClient(tank) || GetEntProp(tank, Prop_Send, "m_zombieClass") != L4D2Infected_Tank) return Plugin_Continue;
...
public Action L4D_OnCThrowActivate(int ability)
{
	...
	throwQueuedAt[client] = GetEngineTime();
	return Plugin_Continue;
}
...
bool ShouldCancelJump(int client)
{
	...
	return (1.5 > GetEngineTime() - throwQueuedAt[client]);
}
